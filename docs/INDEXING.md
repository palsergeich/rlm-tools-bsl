...
|------------|---------------|-------------------|
| `.xml`, `.mdo` | Категорийные таблицы (`object_attributes`, `object_synonyms`, `event_subscriptions` и др.) по `changed_categories` | `form_elements`, `role_rights` |
| `.form` | Только `form_elements` | Категорийные таблицы |
| `.rights` | Только `role_rights` | Категорийные таблицы |
| `.xdto` | Только `xdto_packages` (через category) | `form_elements`, `role_rights` |

`changed_categories` строится только из `.xml`/`.mdo`/`.xdto` — это первый сегмент пути
(например `Catalogs/Номенклатура/Номенклатура.xml` → `Catalogs`).
