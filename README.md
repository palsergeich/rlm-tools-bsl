...
**RAG** — это ткацкий станок на фабрике: мощный, производительный, выдаёт результат промышленного качества. Но требует помещения, настройки, обслуживания и времени на запуск.

**RLM** — это швейная машинка дома: компактная, всегда под рукой, можно взять с собой. Не заменит фабрику, но и не требует её. Подшить, перекроить, разобраться в выкройке — быстро и без подготовки.
...
- Нужен полный граф зависимостей объектов (кто кого вызывает с максимальной глубиной, типы реквизитов по ссылкам) — для этого нужен RAG/графовый MCP с предварительной индексацией
...
```
Пользователь: "Как работает подсистема Спецодежда? Проанализируй по каталогу исходных кодов: D:/project/src/cf"

Агент вызывает rlm_start(path="D:/project/src/cf", query="подсистема Спецодежда")
